...
def minimum_length(arr: Iterable[np.ndarray],
    ...
    arr, arr_padded = arr.astype(np.uint8), arr_padded.astype(np.uint8)

    def _interior_pixel_number(diameter,arr,margin_number,pad_number,dual=True):
        '''
        ...
        se = get_structuring_element(diameter,all_pixel_sides)
        if dual:
            diff_image = binary_open(arr,se)^binary_close(arr,se)
            #cv.morphologyEx(arr,cv.MORPH_OPEN,se)^cv.morphologyEx(arr,cv.MORPH_CLOSE,se)
        else:
            diff_image = binary_open(arr,se)^arr
            #abs(cv.morphologyEx(arr,cv.MORPH_OPEN,se)-arr)
        return _interior_solid_pixel_count(diff_image,margin_number,pad_number,dims)

    if measure_region in minimum_indicator and not with_boundary:
        min_len_padded, status = _search([shorter_pixel_side,shorter_entire_side],
                                         min(all_pixel_sides),
                                         lambda d: _interior_pixel_number(d,arr_padded,margin_number,pad_number),
                                         interior_pixel_threshold)
        return min_len_padded

    if measure_region in solid_indicator+both_indicator+minimum_indicator:
        min_len_solid_padded, status = _search([shorter_pixel_side,shorter_entire_side],
                                               min(all_pixel_sides),
                                               lambda d: _interior_pixel_number(d,arr_padded,margin_number,
                                                                                pad_number,dual=False),
                                              interior_pixel_threshold)
        min_len_solid = min_len_solid_padded

        if pad_mode==None:
            min_len_solid_unpadded, status = _search([shorter_pixel_side,shorter_entire_side],
                                                     min(all_pixel_sides),
                                                     lambda d: _interior_pixel_number(d,arr,margin_number,
    ...
    if measure_region in void_indicator+both_indicator+minimum_indicator:
        min_len_void_padded, status = _search([shorter_pixel_side,shorter_entire_side],
                                              min(all_pixel_sides),
                                              lambda d: _interior_pixel_number(d,1-arr_padded,margin_number,
                                                                               pad_number,dual=False),
                                              interior_pixel_threshold)
        min_len_void = min_len_void_padded

        if pad_mode==None:
            min_len_void_unpadded, status = _search([shorter_pixel_side,shorter_entire_side],
                                                    min(all_pixel_sides),
                                                    lambda d: _interior_pixel_number(d,1-arr,margin_number,
                                                                                     pad_number=np.zeros(dims),
                                                                                     dual=False),
                                                    interior_pixel_threshold)
...
def binary_open(arr,kernel,pad_mode=None):
    pad_width = ((kernel.shape[0],) * 2, (kernel.shape[1],) * 2)
    ((top, bottom), (left, right)) = pad_width
    arr_padded = cv.copyMakeBorder(arr.view(np.uint8), top=top, bottom=bottom, left=left, right=right,
                                   borderType=cv.BORDER_REPLICATE)#.view(bool)

    unpad_width = ((kernel.shape[0] + (kernel.shape[0] + 1) % 2,
                    kernel.shape[0] - (kernel.shape[0] + 1) % 2,),
                   (kernel.shape[1] + (kernel.shape[1] + 1) % 2,
                    kernel.shape[1] - (kernel.shape[1] + 1) % 2,),)
    opened = cv.morphologyEx(src=arr_padded,kernel=kernel.view(np.uint8),op=cv.MORPH_OPEN)
    slices = tuple([slice(pad_lo, dim - pad_hi) for (pad_lo, pad_hi), dim in zip(unpad_width, opened.shape)])
    return opened[slices].astype(bool)


def binary_close(arr,kernel,pad_mode=None):
    pad_width = ((kernel.shape[0],) * 2, (kernel.shape[1],) * 2)
    ((top, bottom), (left, right)) = pad_width
    arr_padded = cv.copyMakeBorder(arr.view(np.uint8), top=top, bottom=bottom, left=left, right=right,
                                   borderType=cv.BORDER_REPLICATE)#.view(bool)
    ...
    closed = cv.morphologyEx(src=arr_padded,kernel=kernel.view(np.uint8),op=cv.MORPH_CLOSE)
    slices = tuple([slice(pad_lo, dim - pad_hi) for (pad_lo, pad_hi), dim in zip(unpad_width, closed.shape)])
    return closed[slices].astype(bool)
...
def _binarize(arr,threshold=0.5):
    ...
    '''
    arr_binarized = arr > threshold*max(arr.flatten())+(1-threshold)*min(arr.flatten())
...
def get_structuring_element(diameter,pixel_size):
    ...
    if len(pixel_size) == 2:
        ...
        z_tick = np.linspace(-rounded_size[2]/2,rounded_size[2]/2,se_shape[2])
        X, Y, Z = np.meshgrid(x_tick, y_tick, z_tick, sparse=True, indexing='ij')
        structuring_element = X**2+Y**2+Z**2 <= diameter**2/4
    else:
...
"""
def _interior_solid_pixel_count(arr,margin_number=np.zeros((2,2),dtype=int),pad_number=np.zeros(2,dtype=int),dims=2):
    '''
...
def _interior_solid_pixel(arr):
    edges = cv.Canny(arr.astype(np.uint8),0,1)
"""
